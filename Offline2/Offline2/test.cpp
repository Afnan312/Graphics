...
h_point rodrigues(h_point a, h_point b, double theta)
{
    ...
    h_point cross=product(sin(rad), cross_product(a, b));
    double dot=dot_product(a, b);

    result.x=a.x*cos(rad)+(1-cos(rad))*dot*a.x+cross.x;
    result.y=a.y*cos(rad)+(1-cos(rad))*dot*a.y+cross.y;
    result.z=a.z*cos(rad)+(1-cos(rad))*dot*a.z+cross.z;
    return result;
}
